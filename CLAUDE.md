...
A digital decision assistant for **SME entrepreneurs**. They answer a 19-question diagnostic (17 scored Likert + 2 classificatory), the model returns:
...
| 10 | **Pivot** — company search + per-company dashboard + 19-question diagnostic | ✅ done |
...
- **Diagnostic questionnaire** — rebuilt against the 19-question diagnostic (Q20 dropped — see diagnostic-schema.ts) in `Value_Intelligence_Questionnaire+contextdata.docx`. Pure qualitative (1–5). Quantitative data is pulled from the AIDA snapshot at submission time, not typed by the entrepreneur.
...
└── web/                       Next.js app
    ├── app/
    │   ├── (marketing)/       public home + /technical deep-dive
    │   ├── (auth)/            login/signup/verify (live — auth is on)
    │   ├── (app)/             /companies, /companies/[taxCode](/diagnostic),
    │   │                      /how-it-works, /method (+ legacy redirects)
    │   └── api/companies/search   typeahead over the AIDA snapshot view
    ├── components/
    │   ├── chrome/            brand mark, session-aware auth CTA, Cmd-K palette
    │   ├── companies/         search bar + results
    │   ├── company-workspace/ the Decision Cockpit (tabs: cockpit/scenario/method)
    │   ├── cockpit/           passport, value bridge, constellation, strategy board
    │   ├── dashboard/         simulation-panel (scenario lab)
    │   ├── diagnostic/        guided 7-step questionnaire (19 questions)
    │   ├── explainer/         /how-it-works narrative
    │   ├── technical/         /technical architecture scenes
    │   ├── marketing/         scene-header (shared by technical scenes)
    │   ├── vip-ui/            in-repo design-system primitives
    │   └── auth/              login/signup forms (live)
    ├── lib/
    │   ├── supabase/          server / service / middleware helpers
    │   ├── scoring/           the 6-stage valuation pipeline
    │   ├── format.ts          ALL money/number formatting helpers
    │   ├── database.types.ts  generated by `supabase gen types`
    │   ├── diagnostic-schema.ts  Zod schema for the 19 questions
    │   ├── demo-data.ts       seeded ACME profile (used by /how-it-works)
...
- **Auth is ON**. `web/lib/supabase/middleware.ts` has `ENFORCE_AUTH_GUARDS = true`. `/companies`, `/dashboard`, `/diagnostic` and `/onboarding` require a session; unauthenticated visitors are redirected to `/login?next=<path>`. The `(app)/layout.tsx` shows the signed-in email + sign-out. Flip the flag to `false` to return to the open demo mode.
- **All money formatting lives in `web/lib/format.ts`** (`formatEurCompact`, `formatThkMillions`, `thkToEur`, …). Don't re-implement €-compaction inside components — import from there. AIDA monetary values are in *thousands* of EUR (`_thk` suffix).
...
web/lib/scoring/
  ...
  metrics.ts           Stage 1: derives ~16 metrics from the 19 inputs
